...
class Pipe:
    ...
    def __init__(self, canvas_group, texture, x, gap_center, gap, scale):
        ...
        with canvas_group:
            ...
            # top pipe (flipped vertically) - use negative-size trick via texture uv flip
            flipped = texture.get_region(0, 0, tw, th)
            flipped.flip_vertical()
            self.top_rect = Rectangle(texture=flipped, pos=(x, 0), size=(self.w, self.h))
        self.set_positions()
    ...
    def set_positions(self):
        ...
        self.top_rect.pos = (self.x, top_y)
...
class GameWidget(FloatLayout):
    ...
    def on_resize(self, *args):
        self.scale = min(self.width / BASE_W, self.height / BASE_H)
        ...
        self.message_rect.size = (mw * self.scale, mh * self.scale)
